...
# Max allowed quantity — anything above is likely a 1000x inflation artifact
...
def snapshot_workbook_sheet(wb, name):
    ws = wb[name]
    sheet = {
        "max_row": ws.max_row,
            ...
            diffs.append(
                {
                    "type": "ROW_COUNT",
                    "sheet": name,
                    "baseline": ws_b.max_row,
                    "current": ws_c.max_row,
                }
            )
...
def save_baseline():
    ...
    lines = [
        f"KOLTAN 13353 approved baseline - {datetime.now().strftime('%Y-%m-%d %H:%M')}",
...
def run_smoke():
    if not BASELINE_VALUES.exists():
        ...
        print("Run  python smoke_test_koltan_13353.py --save  first.")
        return 1

    print("=" * 70)
    print("SMOKE TEST - KOLTAN 13353")
    print("=" * 70)
    ...
    for sheet, sd in sorted(by_sheet.items()):
        ...
        for d in sd[:50]:
            if d["type"] == "CELL_DIFF":
                print(
                    f"  Row {d['row']}, Col {d['col']} ({d['header']}):  "
...
def main():
    ap = argparse.ArgumentParser(description="Smoke test - Koltan 13353")
    ap.add_argument(
        "--save",
        action="store_true",
