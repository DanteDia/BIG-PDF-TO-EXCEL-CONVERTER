...
# No separate Gallo Excel for this case — Visual used for both paths.
VISUAL_PATH = STURMAN_DIR / "2797bolsa_fixed4.xlsx"
GALLO_PATH = VISUAL_PATH  # same file; Gallo-specific sheets absent → Visual-only merge

# Client-approved reference (single "Reporte" sheet, different structure from pipeline)
...
def save_baseline():
    ...
    # Copy client-approved reference into baseline dir
    if CLIENT_FINAL.exists():
        dst = BASELINE_DIR / CLIENT_FINAL.name
        shutil.copy2(str(CLIENT_FINAL), str(dst))
        print(f"  Copied client  : {dst.name}")

    # JSON snapshot — skip huge aux reference sheets
    snap = {}
    ...
    # ---- summary ----
    lines = [
        f"J_STURMAN 2797 approved baseline – {datetime.now().strftime('%Y-%m-%d %H:%M')}",
    ...
    for r in range(2, ws_bol.max_row + 1):
        oper = str(ws_bol.cell(r, cols.get("Tipo Operación", 6)).value or "").lower()
        code = ws_bol.cell(r, cols.get("Cod.Instrum", 7)).value
        especie = ws_bol.cell(r, cols.get("Instrumento", 8)).value
        qty = ws_bol.cell(r, cols.get("Cantidad", 10)).value
        if "ejercicio" in oper or str(code) == "710":
            lines.append(f"  Row {r}: oper={oper}, code={code}, especie={especie}, qty={qty}")
...
def run_smoke():
    if not BASELINE_VALUES.exists():
        ...
        print("Run  python smoke_test_sturman.py --save  first.")
        return 1

    print("=" * 70)
    print("SMOKE TEST — J_STURMAN 2797")
    print("=" * 70)
...
def main():
    ap = argparse.ArgumentParser(description="Smoke test – J_Sturman 2797")
